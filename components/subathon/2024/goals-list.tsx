...
export function GoalsList() {
  return (
    <div className='flex flex-col gap-6 space-y-6 px-4 py-6'>
      ...
      <div className='grid gap-4 sm:p-2 md:p-4 xl:p-4'>
        {goals.map((goal, index) => {
          const cardContent = (
            <Card
              key={index}
              className={`group p-6 transition-transform ${
                goal.completed
                  ? "bg-linear-to-tr from-white to-green-300/20 hover:scale-105 dark:from-green-800/40 dark:to-black"
                  : "bg-linear-to-tl from-gray-100 to-white dark:from-slate-900 dark:to-slate-950"
              }`}
            >
              <div className='flex h-full flex-col space-y-2 lg:space-y-3'>
                <div className='flex items-center justify-between'>
                  ...
                  {goal.completed && (
                    <Badge className='border-slate-50 bg-pink-500 font-medium text-white dark:text-black'>
                      Completed!
                    </Badge>
                  )}
                </div>
                <h4 className='text-sm leading-tight font-semibold text-gray-900 sm:text-lg xl:text-xl 2xl:pt-3 2xl:pl-1 2xl:text-2xl dark:text-gray-200'>
                  {goal.title}
                </h4>
                <p className='flex max-w-4xl flex-1 text-xs leading-snug text-pretty text-gray-700 sm:text-sm xl:text-base 2xl:pl-1 2xl:text-lg dark:text-gray-100'>
                  {goal.description}
                </p>
                ...
                <div className='flex pt-2 font-semibold drop-shadow sm:text-lg md:justify-end xl:text-xl'>
                  {goal.videoLink ? (
                    ...
                    goal.date && (
                      <span className='text-gray-900 italic dark:text-gray-50'>
                        {goal.date}
                      </span>
                    )
                  )}
